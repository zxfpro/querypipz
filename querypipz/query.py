...
class Queryr:
    ...
    def ask(self, prompt: str, similarity_top_k: int = 3, query_type: QueryType = QueryType.Query1):
        """
...
if __name__ == "__main__":
    # Example usage:
    # Build the index (replace with your actual file path)
    # build_index(file_path="./your_document_directory", persist_dir="./my_kb")

    # Initialize Queryr with the persisted index
    qr = Queryr(persist_dir="./obsidian_kb")

    # Example of asking a question
    if qr.index:
        response = qr.ask("What is the main topic of the documents?")
        print("Query Response:", response)

        # Example of retrieving documents
        retrieved_docs = qr.retrieve("important concepts")
        print("\nRetrieved Documents:")
        for doc in retrieved_docs:
            print(f"- {doc.text[:100]}...") # Print first 100 characters of each doc
    else:
        print("Index not available. Cannot perform queries or retrieval.")
